...
var definitions = {
  ...
  MAIN_TEXT: {
    'bg/out-of-range': function(source, defs) {
      var a = t('This BG value was above or below ');
      var b = t("'s threshold for reporting a numerical value. Your actual BG value was higher or lower than displayed.");
      return defs.stitch(a, b, source);
    },
    'animas/bolus/extended-equal-split': function(source, defs) {
      var a = '';
      var b = t(" pumps don\'t capture the details of how combo boluses are split between the normal and extended amounts.");
      return defs.stitch(a, b, source);
    },
    'animas/out-of-sequence': function(source, defs) {
      var a = t("If the data here overlaps, it's because the date/time was changed and ");
      var b = t(" pumps don\'t capture when this happened.");
      return defs.stitch(a, b, source);
    },
    ...
    'animas/bolus/unknown-duration': function(source, defs) {
      var a = t('We know this bolus was canceled, but ');
      var b = t(' pumps do not capture exactly when.');
      return defs.stitch(a, b, source);
    },
    ...
    'carelink/basal/fabricated-from-schedule': function(source, defs) {
      var a = t('We are calculating the basal rates here using the active basal schedule in your pump settings (and applying the percentage of an active temp basal where applicable), but ');
      var b = t(' did not directly provide us with these rate changes.');
      return defs.stitch(a, b, source);
    },
    'carelink/basal/fabricated-from-suppressed': function(source, defs) {
      var a = t('We are deriving the basal rate to display here assuming that your pump resumed to the basal that was active before the pump was suspended, but ');
      var b = t(' did not directly provide us with this rate change.');
      return defs.stitch(a, b, source);
    },
    'carelink/basal/off-schedule-rate': function(source, defs) {
      var a = t('You may have changed pumps recently - perhaps because you had to have your pump replaced due to malfuction. As a result of how ');
      var b = t(" reports the data, we can't be 100% certain of your basal rate here.");
      return defs.stitch(a, b, source);
    },
    'carelink/bolus/missing-square-component': function(source, defs) {
      var a = t('Because of how ');
      var b = t(' reports bolus data, normal and square-wave boluses may not be properly combined to appear as a dual-wave bolus.');
      return defs.stitch(a, b, source);
    },
    'carelink/wizard/long-search': function(source, defs) {
      var a = t('Because of how ');
      var b = t(" reports bolus and wizard data, we can't be 100% certain that the bolus wizard information here (e.g., carbs, suggested dose) corresponds with the bolus.");
      return defs.stitch(a, b, source);
    },
    'insulet/basal/fabricated-from-schedule': function(source, defs) {
      var a = t('We are estimating the duration of the basal rate here using the basal schedule active at download, but ');
      var b = t(' did not directly provide us with this information.');
      return defs.stitch(a, b, source);
    },
    'insulet/bolus/split-extended': function(source, defs) {
      var a = t('Because ');
      var b = t(' represents extended boluses that cross midnight as two boluses, this bolus could be part of a dual-wave bolus, not an independent square-wave bolus as represented here.');
      return defs.stitch(a, b, source);
    },
    ...
    'medtronic/basal/one-second-gap': function(source, defs) {
      var a = t('This basal was extended by one second because the duration and time reported by ');
      var b = t(' do not match up exactly.');
      return defs.stitch(a, b, source);
    },
    'tandem/basal/fabricated-from-new-day': function(source, defs) {
      var a = t('We have fabricated this basal segment from a ');
      var b = t(' new day event; it was not provided directly as basal data');
      return defs.stitch(a, b, source);
    },
    ...
    'basal/unknown-duration': function(source, defs) {
      var a = t('Because of how ');
      var b = t(' reports the data, we could not determine the duration of this basal rate segment.');
      return defs.stitch(a, b, source);
    },
    'stats-insufficient-data': function() {
